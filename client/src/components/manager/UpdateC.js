...
const submitForm = (e) => {
    ...
    axios.patch('/updateCashier', JSON.parse(jsonUpdate)).then(res=> props.history.push('/manager')).catch(err => console.log(err))
    }
...
const SelectMenu = () => {
  console.log(cashier)
    if(load) {
    return cashier.map(cash => <DropdownItem  key={cash.EID} name={cash.Fname} onClick={selectItem}>{cash.Fname}
    </DropdownItem>)
}
}
    ...
    return ( <div>
            ...
            <Form onSubmit={submitForm}>
               <FormGroup className="mt-3">
                ...
                <Input type="text" name="Fname" id="Fname" onChange={handleInputChange} placeholder="Enter new Name" />
                </FormGroup>
                <FormGroup>
                <Label for="Lname">Last Name</Label>
                <Input type="text" name="Lname" id="Lname"  onChange={handleInputChange} placeholder="Enter new Desription" />
                </FormGroup>
