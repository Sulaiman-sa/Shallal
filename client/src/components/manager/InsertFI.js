...
const InsertFI = (props) => {
    useEffect(() => {
      ...
      axios.get('http://localhost:3000/FIID')
            .then(res => {
                setID(res.data)
                setLoad(true)
            })
...
const submitForm = (e) => {
    ...
    if(load && foodCategory[0] && statu[0]){
        ...
        console.log(update)
        axios.put('/insertFI', update).then(res=> props.history.push('/manager')).catch(err => console.log(err))
    }
...
if(!load) {
    ...
    return ( <div>
            ...
            <Form onSubmit={submitForm}>
                ...
                <Input type="number" name="Price" id="Price"  onChange={handleInputChange} placeholder="Enter new Price" />
                </FormGroup>
                <FormGroup>
